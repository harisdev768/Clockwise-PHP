...
class ClockStatusMapper {
    ...
    public function fetchStatus(ClockStatus $status): ClockStatus
    {
        ...
        if (!$clock) {
            return new ClockStatus(); // No active clock found
        }
        ...
        $stmt->bindValue(':user_id', $statusHydrated->getUserId(), \PDO::PARAM_INT);
        $stmt->bindValue(':clock_id', $statusHydrated->getClocked(), \PDO::PARAM_INT);
        $stmt->execute();
